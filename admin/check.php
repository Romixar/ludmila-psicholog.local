...
class Check{// проверка входящих данных
    ...
    public function getPreviewText($mes, &$body){
        $len = strlen($mes);
        // создаем нужные элементы массива для вставки в БД и вывод на сайт
        if($len < 200){
            $head = $mes;// тогда весь отзыв это вступление
            $body = '';
        }
        ...
        if($len > 200){
            ...
            $body = substr($strEnd, strlen($prepost)-1);
            
            return $str160.''.$prepost;//верну заголов сложить из первых двух кусков
        }
